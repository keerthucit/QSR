...
public class RetailThumbNailBuilder {
    ...
    public RetailThumbNailBuilder(int width, int height) {
        init(width, height, null);
    }
    
    public RetailThumbNailBuilder(int width, int height, Image imgdef) {
        init(width, height, imgdef);
      
    }
    
    public RetailThumbNailBuilder(int width, int height, String img, String name) {
        
        Image defimg;

        try {
            init(width, height, ImageIO.read(getClass().getClassLoader().getResourceAsStream(img)),name);
        } catch (Exception fnfe) {
            init(width, height, null,null);
        }                 
    }    
    
    private void init(int width, int height, Image imgdef, String name) {
        m_width = width;
        m_height = height;
        buttonName=name;
        if (imgdef == null) {
    ...
    public Image getThumbNailText(Image img, String text) {
        ...
        label.setBackground(new Color(105,140,86));
     // label.setBackground(new Color(0, 0, 0, 0));
        label.setPreferredSize(new Dimension(110, 57));
...
//        Color c2 = new Color(0xff, 0xff, 0xff, 0xd0);
        ...
        if(buttonName=="Category"){
          // c1 = new Color(105,140,86);
          // c2 = new Color(105,140,86);
            c1 = new Color(0xff, 0xff, 0xff, 0x40);
         c2 = new Color(0xff, 0xff, 0xff, 0xd0);
        }else if(buttonName=="Subcategory"){
//           c1 = new Color(124,184,158);
//           c2 = new Color(124,184,158);

             c1 = new Color(0xff, 0xff, 0xff, 0x40);
         c2 = new Color(0xff, 0xff, 0xff, 0xd0);
        }
        else{
        //   c1 = new Color(69,83,155);
      //     c2 = new Color(69,83,155);
              c1 = new Color(0xff, 0xff, 0xff, 0x40);
         c2 = new Color(0xff, 0xff, 0xff, 0xd0);
        }
