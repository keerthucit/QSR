...
public abstract class JPaymentSelect extends javax.swing.JDialog
    ...
    private ArrayList<VouchersList> cashlist = new ArrayList<VouchersList>();
    private ArrayList<VouchersList> checklist = new ArrayList<VouchersList>();
    private ArrayList<VouchersList> voucherlist = new ArrayList<VouchersList>();
    private JPaymentInterface jpayinterface;
    private JPaymentPaper paymentPaper;
    ...
    public List<PaymentInfo> getSelectedPaymentsSplits() {
        ...
        for (final PaymentInfo p : m_aPaymentInfo.getPayments()) {
            PaymentInfo ppp = new PaymentInfo() {
                ...
                public ArrayList<VouchersList> getPaymentSplits() {
                    ArrayList<VouchersList> av = new ArrayList<VouchersList>();
                    ...
                    if (p.getName().equals("Voucher")) {
                        av.addAll(voucherlist);
                    }
                    return av;
                }
            };
    ...
    public static void setAddEnabled(boolean value) {
    ...
    protected void addTabPayment(JPaymentCreator jpay) {
        if (app.getAppUserView().getUser().hasPermission(jpay.getKey())) {
            ...
            jpayinterface = payments.get(jpay.getKey());
...
//            setOKEnabled(false);
    ...
    private void storeCashDetails(PaymentInfo p) {
        if (p.getName().equals("Cash")) {
            cashlist.add(new VouchersList(p.getVoucherNo(), p.getTotal()));
        }
        if (p.getName().equals("Cheque")) {
            checklist.add(new VouchersList(p.getVoucherNo(), p.getTotal()));
        }
        if (p.getName().equals("Voucher")) {
            voucherlist.clear();
            ArrayList<String> vnos = paymentPaper.finder.checknodetails;
            ArrayList<Double> amts = paymentPaper.finder.checkamtdetails;
            for (int i = 0; i < vnos.size(); i++) {
                voucherlist.add(new VouchersList(vnos.get(i), amts.get(i)));
            }

        }
    }

    public ArrayList<VouchersList> returnCashDetails() {
        ArrayList<VouchersList> vs = new ArrayList<VouchersList>();
        //vs.addAll(cashlist);
        //vs.addAll(checklist);
        vs.addAll(voucherlist);
        return vs;
    }
    ...
    public class JPaymentPaperCreator implements JPaymentCreator {
        ...
        public JPaymentInterface createJPayment() {
            paymentPaper = new JPaymentPaper(JPaymentSelect.this, "paperin", app);
...
//    }
    ...
    private void printState2() {
        ...
        double totalPayVoucher = 0;
        for (int i = 0; i < m_aPaymentInfo.getPayments().size(); i++) {

            if (m_aPaymentInfo.getPayments().get(i).getName().equals("Cash")) {
                totalPayCash = totalPayCash + m_aPaymentInfo.getPayments().get(i).getTotal();
            }
            if (m_aPaymentInfo.getPayments().get(i).getName().equals("Cheque")) {
                totalPayCheque = totalPayCheque + m_aPaymentInfo.getPayments().get(i).getTotal();
            }
            if (m_aPaymentInfo.getPayments().get(i).getName().equals("Voucher")) {
                totalPayVoucher = m_aPaymentInfo.getPayments().get(i).getTotal();
            }
        }

        double pay = totalPayCash + totalPayCheque + totalPayVoucher;
        m_jRemaininglEuros.setText(Formats.CURRENCY.formatValue(new Double(m_dTotal - pay)));
        m_jButtonRemove.setEnabled(!m_aPaymentInfo.isEmpty());
        m_jTabPayment.setSelectedIndex(0); // selecciono el primero
        ((JPaymentInterface) m_jTabPayment.getSelectedComponent()).activate(customerext, m_dTotal - pay, m_sTransactionID);
    }
    ...
    private void m_jButtonAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_m_jButtonAddActionPerformed
        ...
        if (returnPayment != null) {
            ...
            storeCashDetails(returnPayment);
            printState2();
        }
...
//        paymentPaper.finder = null;
    ...
    public static javax.swing.JButton m_jButtonAdd;
