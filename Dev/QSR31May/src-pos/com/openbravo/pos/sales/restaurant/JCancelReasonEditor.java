...
//            }
            for(int i=0;i<tinfoLocal.getLinesCount();i++){
                   localDlReceipts.insertCancelledKot(tinfoLocal.getId(),tinfoLocal.getDate(),tinfoLocal.getTicketId(),panelLines.get(i).getProductID(), "Y",(1*panelLines.get(i).getMultiply()) ,kotTicket,"Y",reason,reasonId,tinfoLocal.getPlaceId(),tinfoLocal.getUser().getId());
            }
             try {
