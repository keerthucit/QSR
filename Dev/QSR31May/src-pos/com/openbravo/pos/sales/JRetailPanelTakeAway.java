...
import com.openbravo.pos.forms.BogoInfo;
...
import static com.openbravo.pos.sales.JRetailPanelTicket.taxModel;
...
public abstract class JRetailPanelTakeAway extends JPanel implements JPanelView, BeanFactoryApp, RetailTicketsEditor {
    ...
    String[] args;
    boolean settlementSucess = false;
    ...
    public java.util.ArrayList<BuyGetPriceInfo> pdtBuyGetPriceList = null;
    ...
    public java.util.ArrayList<BogoInfo> bogoList;
    int bogoCount = 0;
    boolean bogoMainItem = false;
    boolean leastValuePromo = false;
    String bogoPrId = null;
    java.util.List<RetailTicketLineInfo> bogoSecondaryItems = null;
    private Map<String, Double> bogoMap = new HashMap<String, Double>();
...
//         }else{
        ...
        cash = new PaymentInfoCash(ticket.getTotal(), ticket.getTakeAwayTotal(), 0);
        if (cash != null) {
...
//         }else{
        ...
        card = new PaymentInfoCard(m_oTicket.getTotal(), m_oTicket.getTakeAwayTotal());
        if (card != null) {
...
//    }
    ...
    private void populateDiscount(Map<String, DiscountInfo> discountMap) {
        ...
        if (discountMap != null) {
            ...
            for (int i = 0; i < m_oTicket.getLinesCount(); i++) {
                ...
                String parentCatId = m_oTicket.getLine(i).getCatId();
                String discount = "";
    ...
    private String checkPromoType(String promoType) {
        String prId = null;
        for (PromoRuleIdInfo pr : promoRuleIdList) {
            if (pr.getPromoType().equals(promoType)) {
                prId = pr.getpromoRuleId();
            }
        }
        return prId;
    }
    public static Comparator<RetailTicketLineInfo> productPriceComparator = new Comparator<RetailTicketLineInfo>() {
        @Override
        public int compare(RetailTicketLineInfo l1, RetailTicketLineInfo l2) {
            Double productQty1 = l1.getPrice();
            Double productQty2 = l2.getPrice();

            return productQty1.compareTo(productQty2);
        }
    };

    private void checkBogoQty() {
        bogoSecondaryItems = new ArrayList();
        int index = 0;
        int removeIndex = 0;
        int lineIndex = 0;
        //    boolean removeIndexStatus=false;
        for (RetailTicketLineInfo line : m_oTicket.getLines()) {
            if (line.getPromoRule() != null) {
                if (line.getPromoRule().get(0).getMainBogo().equals("N")) {
                    if (line.getPromoRule().get(0).getpromoId().equals(bogoPrId)) {
                        index = index + (int) line.getMultiply();
                        if (index > 2) {
                            removeTicketLine(lineIndex);
                            break;
                        } else {
                            bogoMap.put(line.getProductName(), line.getMultiply());
                            bogoSecondaryItems.add(line);
                        }
                    }
                }
            }
        ...
        //  System.out.println("bogoSecondaryItems"+bogoSecondaryItems.get(0).getMultiply());
        if (!bogoSecondaryItems.isEmpty()) {
            if ((bogoSecondaryItems.size() == 2 || bogoSecondaryItems.get(0).getMultiply() == 2)) {
                Collections.sort(bogoSecondaryItems, productPriceComparator);
                bogoSecondaryItems.get(0).setOfferDiscount(bogoSecondaryItems.get(0).getPrice());
                bogoSecondaryItems.get(0).setPromodiscountPercent(100);
            }
        }
...
//                itemName.setText(str);
    ...
    public void paymentDetail(double cashAmount, double cardAmount, double voucherAmount, double vCloudAmount) {
        m_aPaymentInfo = new PaymentInfoList();
...
//        m_jCboCustName.setModel(new ComboBoxValModel(cusNames));
    ...
    public void activate() {
        ...
        isServiceChargeApplicable = m_App.getProperties().getProperty("machine.servicecharge");
        if (isServiceChargeApplicable.equals("true")) {
            m_jServiceChargeLbl.setVisible(true);
            m_jServiceCharge.setVisible(true);
        } else {
...
//            }
//
        m_jLblItemCode.setVisible(false);
        m_jTxtItemCode.setVisible(false);
        m_jLblItemName.setVisible(false);
        m_jCboItemName.setVisible(false);
        jButtonAddon.setVisible(false);
        jLabel1.setVisible(false);
        m_jLblBillNo.setVisible(false);
    ...
    private void loadPromotion() {
        ...
        //  System.out.println("loadMenu-----" + "--current--" + currentTime + "----" + promotionList.size());

        //next promotion start and end time testing testing
        for (CampaignIdInfo p : promotionList) {

            //  System.out.println("loadMenu-----" + currentTime + " " + p.getStartTime());
            if (p.getStartTime().equals(currentTime) || p.getEndTime().equals(currentTime)) {
                //   System.out.println("peak time for next promotion-----");
                //   try {
                //   String day = getWeekDay();
                //   promotionList=dlSales.getActivePromotionCampList(day);
                setPromotionData();
...
//                }
    ...
    private void setPromotionData() {
        ...
        try {
            campaignIdList = (ArrayList<CampaignIdInfo>) dlSales.getCampaignId(day);
            if (campaignIdList.size() != 0) {
                //endPromoTime=campaignIdList.get(0).getEndTime();
            }
        } catch (BasicException ex) {
            Logger.getLogger(JRetailPanelTicket.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        if (campaignIdList.size() != 0) {
            ...
            if (campaignIdList != null) {
                try {
                    promoRuleIdList = (ArrayList<PromoRuleIdInfo>) dlSales.getPromoRuleId(Id);
                    int LeastPromotionCount = dlSales.getLeastPromo(Id);
                    if (LeastPromotionCount == 0) {
                        m_jCalculatePromotion.setEnabled(false);
                    } else {
                        m_jCalculatePromotion.setEnabled(true);
                    }
                    int bogoCount = dlSales.getBogoPromo(Id);
                    System.out.println("bogoCount testing " + bogoCount);
                    if (bogoCount == 0) {
                        m_jCalculateBogo.setEnabled(false);
                    } else {
                        m_jCalculateBogo.setEnabled(true);
                    }
                    System.out.println("promoRuleIdList---setPromotionDatass------------" + promoRuleIdList.size() + "-----Id" + Id);
...
//           m_jTable.setText(m_oTicket.getName(m_oTicketExt) );
    ...
    private void showProductPanel() {
        ...
        for (ProductInfoExt prod : product) {
            tnbbutton = new ThumbNailBuilderPopularItems(110, 57, "com/openbravo/images/bluepopulartoit.png");
            jcurrTab.addPopularItemButton(new ImageIcon(tnbbutton.getThumbNailText(prod.getImage(), getProductLabel(prod))), new SelectedAction(prod));
        }
...
//                calculateServiceCharge();
            ...
            System.out.println("printPromoDiscount===" + m_oTicket.printPromoDiscount());
...
//            }
    ...
    private void addTicketLine(ProductInfoExt oProduct, double dMul, double dPrice) {
        System.out.println("bogoPrId=====" + bogoPrId);
        kotaction = 1;
        TaxInfo tax = null;
        //restricting to aeriated beverages and drinks to fetch exempt tax 01/09/2016
        if (("QSR-Ahmedabad").equals(m_App.getProperties().getStoreName())) {
            if ((oProduct.getCategoryID().equals("856E65D2A7464D2B924CDF2D0C8BDC03") || oProduct.getCategoryID().equals("B746DBCB1FC643269B4F2999001445F7"))) {
                tax = taxeslogic.getTaxInfo(oProduct.getTaxCategoryID(), m_oTicket.getCustomer(), "Y", "Y");
            } else {
                tax = taxeslogic.getTaxInfo(oProduct.getTaxCategoryID(), m_oTicket.getCustomer(), "Y", "N");
            }
        } else {
...
//        boolean add=true;
//        if(!bogoMap.isEmpty()){
//        if(bogoMap.containsKey(oProduct.getName()))    {
//            System.out.println("bogoMap-----"+bogoMap.get(oProduct.getName()));
//        if(bogoMap.get(oProduct.getName())>=2 ){
//           add=false; 
//        }
//        }
//        }
        //   if(add){
        if (bogoMainItem) {
            addTicketLine(new RetailTicketLineInfo(oProduct, dMul, dPrice, promoRuleIdList, dlSales, m_oTicket, m_ticketlines, this, tax, 0, oProduct.getName(), oProduct.getProductType(), oProduct.getProductionAreaType(), (java.util.Properties) (oProduct.getProperties().clone()), addonId, 0, null, 0, null, null, null, charge, sertax, oProduct.getParentCatId(), oProduct.getPreparationTime(), null, sbTax, "Y"));
        } else {
            addTicketLine(new RetailTicketLineInfo(oProduct, dMul, dPrice, promoRuleIdList, dlSales, m_oTicket, m_ticketlines, this, tax, 0, oProduct.getName(), oProduct.getProductType(), oProduct.getProductionAreaType(), (java.util.Properties) (oProduct.getProperties().clone()), addonId, 0, null, 0, null, null, null, charge, sertax, oProduct.getParentCatId(), oProduct.getPreparationTime(), null, sbTax, "N"));
        }
        //  }
        bogoMainItem = false;
...
//            TaxInfo tax = taxeslogic.getTaxInfo(tcid, m_oTicket.getCustomer(), "Y","N");
        ...
        try {
            ...
            if (oProduct == null) {
                ...
                if (m_jaddtax.isSelected()) {
                    ...
                    TaxInfo tax = taxeslogic.getTaxInfo(oProduct.getTaxCategoryID(), m_oTicket.getCustomer(), "Y", "N");
...
////    }
    ...
    public void settleBill(double totalBillValue, double cash, double card, double voucherAmount, double vCloudAmount) {
        this.totalBillValue = totalBillValue;
        paymentDetail(cash, card, voucherAmount, vCloudAmount);
...
//           showMessage(this,"Stock Reconciliation in Progress. Please continue after sometime.");
//         }else{
        if (totalBillValue == 0 && m_oTicket.getTakeAwayTotal() != 0) {
            showMessage(this, "Please enter the tender types");
            setClosePayment(false);
        } else if (totalBillValue < m_oTicket.getTakeAwayTotal()) {
...
//        if (m_App.getAppUserView().getUser().hasPermission("sales.Total")) {
    ...
    public void saveReceipt(RetailTicketInfo ticket, String inventoryLocation, String posNo, String storeName, String ticketDocument, ArrayList<BuyGetPriceInfo> priceInfo, String chequeNos, String deliveryBoy, double advanceissued, double creditAmount, Object ticketext, String status, String file, String isCredit, String nonChargable) {
        ...
        try {
            ...
            settlementSucess = true;
            setClosePayment(true);
        } catch (Exception ex) {
            settlementSucess = false;
            setClosePayment(false);
...
//         itemName.setText("");
    ...
    private java.util.List<TicketLineConstructor> getAllLines(RetailTicketInfo ticket, Object ticketext) {
        ...
        allLines.add(new TicketLineConstructor(m_App.getProperties().getProperty("machine.address4")));
        allLines.add(new TicketLineConstructor(m_App.getProperties().getProperty("machine.address5")));
        allLines.add(new TicketLineConstructor(m_App.getProperties().getProperty("machine.address6")));
        ...
        allLines.add(new TicketLineConstructor("Chk:" + getSpaces(1) + ticket.getTicketId() + getSpaces(4) + "Gst:" + getSpaces(1) + ticket.getNoOfCovers()));
...
//            }else{
        ...
        //  String serviceTax = Formats.DoubleValue.formatValue(ticket.getServiceTax()-ticket.getServiceTaxLeastAmount());
        // String swachBharatTax = Formats.DoubleValue.formatValue(ticket.getSwatchBharatTax()-ticket.getSbTaxLeastAmount());
        String discount = Formats.DoubleValue.formatValue(ticket.getLineDiscount());
        ...
        String total = ticket.printTakeAwayTotal();
...
//                 allLines.add(new TicketLineConstructor(ticket.getServiceTaxes().get(i).getTaxInfo().getName() + getSpaces(19 - Formats.DoubleValue.formatValue(ticket.getServiceTaxes().get(i).getRetailServiceTax()).length()) + (Formats.DoubleValue.formatValue(ticket.getServiceTaxes().get(i).getRetailServiceTax()-ticket.getServiceTaxLeastAmount()))));
//              }
//          }
//
//      }
...
//                 allLines.add(new TicketLineConstructor(ticket.getSwachBharatTaxes().get(i).getTaxInfo().getName() + getSpaces(13 - Formats.DoubleValue.formatValue(ticket.getSwachBharatTaxes().get(i).getRetailSwachBharatTax()).length()) + (Formats.DoubleValue.formatValue(ticket.getSwachBharatTaxes().get(i).getRetailSwachBharatTax()-ticket.getSbTaxLeastAmount()))));
//              }
//          }
//
//      }
...
//    }
    ...
    private void printTicket(String sresourcename, RetailTicketInfo ticket, Object ticketext) {
        ...
        int count = 0;
        com.openbravo.pos.printer.printer.ImageBillPrinter printer = new ImageBillPrinter(30, 10);
...
//        }
    ...
    private void initComponents() {
        ...
        m_jCalculateBogo = new javax.swing.JButton();
        ...
        m_jServiceChargeLbl = new javax.swing.JLabel();
        ...
        jPanel1.setPreferredSize(new java.awt.Dimension(620, 3));
        ...
        jPanel1.add(m_jbtnPrintBill, new org.netbeans.lib.awtextra.AbsoluteConstraints(385, 0, -1, 10));
        ...
        jPanel1.add(m_jSettleBill, new org.netbeans.lib.awtextra.AbsoluteConstraints(480, 0, -1, 10));
        ...
        jPanel1.add(m_jSplitBtn, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 0, -1, 20));
        ...
        jPanel1.add(m_jBtnCancelBill, new org.netbeans.lib.awtextra.AbsoluteConstraints(180, 0, -1, 10));
        ...
        jPanel1.add(m_jBtnBillOnHold, new org.netbeans.lib.awtextra.AbsoluteConstraints(90, 0, 90, 10));
        ...
        m_jPanelBag.setPreferredSize(new java.awt.Dimension(800, 5));
        ...
        jLabel3.setIcon(new javax.swing.ImageIcon(getClass().getResource("/com/openbravo/pos/templates/1header-burritoboys.png"))); // NOI18N
        ...
        jPanel6.add(m_jLblCurrentDate, new org.netbeans.lib.awtextra.AbsoluteConstraints(520, 45, 180, 45));
        ...
        jPanel6.add(m_jLblTime, new org.netbeans.lib.awtextra.AbsoluteConstraints(710, 45, 230, 45));
        ...
        jPanel6.add(m_jLogout, new org.netbeans.lib.awtextra.AbsoluteConstraints(950, 47, -1, -1));
        ...
        jPanel10.setPreferredSize(new java.awt.Dimension(803, 5));
        ...
        jPanel10.add(jButtonAddon, new org.netbeans.lib.awtextra.AbsoluteConstraints(610, 0, 90, 5));
        ...
        jPanel10.add(jButton2, new org.netbeans.lib.awtextra.AbsoluteConstraints(750, 0, 90, 5));
        ...
        jPanel10.add(jButton3, new org.netbeans.lib.awtextra.AbsoluteConstraints(845, 0, 90, 5));
        ...
        jPanel10.add(m_jCboItemName, new org.netbeans.lib.awtextra.AbsoluteConstraints(370, 10, 130, -1));
        ...
        jPanel2.setPreferredSize(new java.awt.Dimension(61, 500));
        ...
        jPanel2.add(m_jPlus, new org.netbeans.lib.awtextra.AbsoluteConstraints(5, 0, -1, -1));
        ...
        jPanel2.add(m_jMinus, new org.netbeans.lib.awtextra.AbsoluteConstraints(5, 47, -1, -1));
        ...
        jPanel2.add(m_jEditLine, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 422, 51, 20));
        ...
        jPanel2.add(m_jCalculatePromotion, new org.netbeans.lib.awtextra.AbsoluteConstraints(5, 235, -1, -1));
        ...
        jPanel2.add(m_jAction, new org.netbeans.lib.awtextra.AbsoluteConstraints(173, 242, -1, 14));
        ...
        jPanel2.add(m_jDelete, new org.netbeans.lib.awtextra.AbsoluteConstraints(5, 94, -1, -1));

        m_jKot.setIcon(new javax.swing.ImageIcon(getClass().getResource("/com/openbravo/images/TKOT copy.png"))); // NOI18N
        m_jKot.setToolTipText("Pay");
        ...
        jPanel2.add(m_jKot, new org.netbeans.lib.awtextra.AbsoluteConstraints(5, 188, -1, -1));
        ...
        jPanel2.add(m_jBtnServed, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 450, 51, 20));
        ...
        jPanel2.add(m_jEraser, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 465, 51, 20));

        m_jBtnDiscount.setIcon(new javax.swing.ImageIcon(getClass().getResource("/com/openbravo/images/tpersentage.png"))); // NOI18N
        ...
        jPanel2.add(m_jBtnDiscount, new org.netbeans.lib.awtextra.AbsoluteConstraints(5, 141, 51, 42));

        m_jCalculateBogo.setIcon(new javax.swing.ImageIcon(getClass().getResource("/com/openbravo/images/bogo.jpg"))); // NOI18N
        m_jCalculateBogo.setMnemonic('f');
        m_jCalculateBogo.setFocusPainted(false);
        m_jCalculateBogo.setFocusable(false);
        m_jCalculateBogo.setMargin(new java.awt.Insets(8, 14, 8, 14));
        m_jCalculateBogo.setPreferredSize(new java.awt.Dimension(51, 42));
        m_jCalculateBogo.setRequestFocusEnabled(false);
        m_jCalculateBogo.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                m_jCalculateBogoActionPerformed(evt);
            }
        });
        jPanel2.add(m_jCalculateBogo, new org.netbeans.lib.awtextra.AbsoluteConstraints(5, 282, -1, -1));
        ...
        jTaxPanel.add(m_jServiceTax, new org.netbeans.lib.awtextra.AbsoluteConstraints(120, 40, 80, 23));
        ...
        m_jServiceChargeLbl.setText("SERVICE CHARGE");
        jTaxPanel.add(m_jServiceChargeLbl, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 93, 120, 30));
        ...
        jTaxPanel.add(m_jServiceCharge, new org.netbeans.lib.awtextra.AbsoluteConstraints(120, 100, 80, 23));
...
//        }
    ...
    private void m_jDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_m_jDeleteActionPerformed
        ...
        if (i < 0) {
            ...
            if (!bogoMap.isEmpty()) {
                if (bogoMap.containsKey(m_oTicket.getLine(i).getProductName())) {
                    bogoMap.remove(m_oTicket.getLine(i).getProductName());
                }
                if (("Y").equals(m_oTicket.getLine(i).getIsBogoMain())) {
                    for (RetailTicketLineInfo line : bogoSecondaryItems) {
                        System.out.println(" m_oTicket.getLine(i)." + m_oTicket.getLine(i).getIsBogoMain());
                        line.setOfferDiscount(0);
                    }
                    bogoMap.clear();
                    bogoPrId = null;
                    m_oTicket.setPromoAction(false);
                }
            }
            ...
            String promoType = m_oTicket.getLine(i).getPromoType();
            String isCrossProduct=m_oTicket.getLine(i).getIsCrossProduct();
            if (kotStatus != 0) {
                if (roleName.equals("Admin") || roleName.equals("Cashier")) {
                    ...
                    if (JReasonEditor.getCancel() == true) {
                        ...
                        try {
                            if (promoType.equals("SIBG") && leastValuePromo == true&& isCrossProduct.equals("N")) {
                                checkLeastValuePromotion();
                            }
                        } catch (Exception e) {
                        }
                        ...
                        if (m_oTicket.getLinesCount() == 0) {
                            ...
                            m_jPromoDiscount.setText("Rs. 0.00");
                ...
                removeTicketLine(i);
                //    System.out.println("m_oTicket.getLine(i).getPromoList().get(0).getPromoType()--"+m_oTicket.getLine(i).getPromoList().get(0).getPromoType());
    ...
    private void m_jPlusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_m_jPlusActionPerformed
        boolean NopromotionItem = true;
        int i = m_ticketlines.getSelectedIndex();
        if (m_oTicket.getLine(i).getPromoType().equals("BOGO")) {
//        if(!bogoMap.isEmpty()){
//        if(bogoMap.containsKey(m_oTicket.getLine(i).getProductName()))    {
            NopromotionItem = false;
//             }
//        }
        } else if (m_oTicket.getLine(i).getPromoType().equals("SIBG")) {
            if (m_oTicket.getLine(i).getPromoRule().get(0).getIsCrossProduct().equals("N") && m_oTicket.getLine(i).getPromoRule().get(0).getIsSku().equals("N")) {
                //    if(m_oTicket.isPromoAction()){
                NopromotionItem = false;
                //}
            }
//else{
...
//         }
        ...
        }//dont allow to plus if there is bogo item or least value item
        if (NopromotionItem) {
            RetailTicketLineInfo newline = new RetailTicketLineInfo(m_oTicket.getLine(i));
...
//                    addTicketLine(product, 1.0, product.getPriceSell());
    ...
    public void checkLeastValuePromotion() {
        leastValuePromo = true;
...
//                m_oTicket.setServiceTaxLeastAmount(serviceTaxAmount);
//                m_oTicket.setSbTaxLeastAmount(sbtaxAmount);
    ...
    private void calculateLeastValueDiscount(ArrayList<BuyGetPriceInfo> pdtLeastPriceList1) {
        for (final RetailTicketLineInfo l : m_oTicket.getLines()) {
            System.out.println("pdtBuyGetPriceLis--------checkvalue---" + pdtBuyGetPriceList.size() + "===" + pdtBuyGetPriceList);
            if (pdtBuyGetPriceList.size() != 0) {
                ...
                System.out.println("enetre ----- sibg----");
                if (l.getPromoType().equals("SIBG")) {
                    l.setPromodiscountPercent(0);
                    l.setPromoType("SIBG");
                    l.setOfferDiscount(0);
                }
            }
        }
    }
...
//            }
    ...
    private void setProductionArea() {
        try {
            String sessionId = null;
            java.util.List<BeanInfo> takeawayInfo = dlReceipts.getTakeAwayInfoList();
            tableId = takeawayInfo.get(0).getId();
            ...
            for (int j = 0; j < printerInfo.size(); j++) {
                for (int i = 0; i < m_oTicket.getLinesCount(); i++) {
                    if (printerInfo.get(j).getProductionAreaType().equals(m_oTicket.getLine(i).getProductionAreaType())) {
                        m_oTicket.getLine(i).setProductionArea(printerInfo.get(j).getProductionArea());
                    }
                }
            }
        } catch (BasicException ex) {
    ...
    private synchronized void printRetailKotTicket(String sresourcename, RetailTicketInfo ticket, java.util.List<RetailTicketLineInfo> kot, Object ticketExt, java.util.List<ProductionPrinterInfo> printerInfo, int kotTicket) {
        ...
        for (int j = 0; j < printerInfo.size(); j++) {
            ...
            if (uniqueProductionAreas.size() != 0) {
                ...
                try {
                    //    if (storeLocation.equals("BlrIndranagar") || storeLocation.equals("BlrKoramangala") || storeLocation.equals("Chennai")) {
...
//        }
    ...
    private java.util.List<TicketLineConstructor> getRetailAllLines(RetailTicketInfo ticket, Object ticketext, java.util.List<RetailTicketLineInfo> kot, int kotTicket) {
        ...
        allLines.add(new TicketLineConstructor("Time: " + (ticket.printTime()) + getSpaces(16 - (ticket.printTime().length()))));
...
//                            calculateServiceCharge();
    ...
    private void m_jSettleBillActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_m_jSettleBillActionPerformed
        System.out.println("split bill sharedid testing" + m_oTicket.getSplitSharedId());
        ...
        if (!m_oTicket.isPrinted()) {
            ...
            logger.info("Bill is Modified after previous print. So please print it agai");
            return;
        }
...
//            }
//           System.out.println("printerissue"+kotprintIssue);
//             //  RetailTicketInfo mTicket= m_oTicket;
//                //   System.out.println("panelLines size in kot pre"+mTicket.getLinesCount());
//                 //differentiating kot items and non kot items
//        //        RetailTicketInfo info = getActiveTicket();
//        //        java.util.List<RetailTicketLineInfo> panelLines = info.getLines();
//        //              for(int i=0;i<info.getLinesCount();i++)
//        //              {
//        //                  info.getLine(i).setIsKot(1);
//        //              if(info.getLine(i).getPreparationStatus()!=3) {
//        //                  info.getLine(i).setPreparationStatus(4);
//        //                  paintTicketLine(i,  info.getLine(i));
//        //              }
//        //              }Bartender
    ...
    public void settleBill() {
        ...
        if (m_oTicket.getLinesCount() != 0) {
            try {
                ...
                logger.info("kotaction is 1" + settlementSucess);
                if (settlementSucess) {
...
//                            calculateServiceCharge();
                    if (printerType.equals("false")) {
                        ...
                        printReceiptTicket(file, m_oTicket, m_oTicketExt);
                    }
                    closeInvoiceTicket(m_oTicket);
                    ...
                    m_oTicket.resetPayments();
                    m_jServiceCharge.setText("");
                    m_jPromoDiscount.setText("");
                    activate();
                    logger.info("After activate method");
                    logger.info("Order No." + m_oTicket.getOrderId() + " deleting shared ticket after settle bill");
                    m_ticketsbag.deleteTicket();
                    leastValuePromo = false;
                }
...
//        }
    ...
    private void m_jCalculateBogoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_m_jCalculateBogoActionPerformed
        bogoPrId = checkPromoType("BOGO");
        ProductInfoExt product = null;
        if (bogoPrId != null) {
            m_oTicket.setPromoAction(true);
            if (menuStatus.equals("false")) {
                try {
                    product = dlSales.getBogoProduct(bogoPrId);
                } catch (BasicException ex) {
                    Logger.getLogger(JRetailPanelTakeAway.class.getName()).log(Level.SEVERE, null, ex);
                }
            } else {
                try {
                    ...
                    product = dlSales.getMenuBogoProduct(menuId, bogoPrId);
                } catch (BasicException ex) {
                    Logger.getLogger(JRetailPanelTakeAway.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
            if (product != null) {
                bogoMainItem = true;
                addTicketLine(product, product.getMultiply(), product.getMrp());

            }

        } else {
            showMessage(this, "There is no Bogo Promotion running now");
        }

    }//GEN-LAST:event_m_jCalculateBogoActionPerformed
...
//        }
    ...
    private javax.swing.JButton m_jCalculateBogo;
    ...
    private javax.swing.JLabel m_jServiceChargeLbl;
